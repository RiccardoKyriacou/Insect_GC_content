...
parse.add_argument("-g","--genomes",type=str, help="ncbi xml file with genome info",required=True)
parse.add_argument("-c","--genes",type=str, help="ensembl csv file with gene annotation info",required=True)
parse.add_argument("-s","--group",type=str, help="file of species names to download",required=False)
parse.add_argument("-t","--threads",type=int, help="file of species names to download",required=False)
...
if args.threads:
    if len(genome_dict) > 0:
        Parallel(n_jobs=args.threads)(delayed(genome_download)(k, v) for k, v in genome_dict.items())  